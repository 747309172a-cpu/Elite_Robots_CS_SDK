...
#include <EliteC/Dashboard/elite_dashboard_c.h>
...
template <typename Fn>
elite_c_status_t run_string_out(elite_dashboard_handle_t* handle, char* out_buffer, int32_t buffer_len,
                                int32_t* out_required_len, Fn&& fn) {
    if (!handle) {
        set_global_error("handle is null");
        return ELITE_C_STATUS_INVALID_ARGUMENT;
    }
...
elite_c_status_t elite_dashboard_connect(elite_dashboard_handle_t* handle, const char* ip, int32_t port,
                                         int32_t* out_success) {
    if (!ip || !out_success) {
...
elite_c_status_t elite_dashboard_popup(elite_dashboard_handle_t* handle, const char* arg, const char* message, int32_t* out_success) {
    if (!arg) {
...
elite_c_status_t elite_dashboard_is_configuration_modify(elite_dashboard_handle_t* handle, int32_t* out_modified) {
    return run_bool_out(handle, out_modified, "out_modified is null",
                        [&]() { return handle->dashboard->isConfigurationModify(); });
}
